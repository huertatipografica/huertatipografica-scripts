...
iiMatraPres=['fa-deva','f_ra-deva','jha-deva.loclNEP','ka-deva','k_va-deva','k_ra-deva','k_ra-deva.alt','k_ka-deva','k_la-deva.loclMAR','k_ta-deva','k_t_ta-deva','pha-deva','ph_ra-deva','ph_la-deva.loclMAR','qa-deva','ra_uuMatra-deva','zha-deva.loclNEP','rVocalic-deva','rrVocalic-deva']
...
def create_otfeature( featurename = "temp", 
...
def create_prefix( featurename = "temp", 
                      featurecode = "# empty feature code", 
                      targetfont  = font,
                      codesig     = "DEFAULT-CODE-SIGNATURE" ):
	"""
...
def widthSum(font,list):
	...
	if font.glyphs[item].layers[0].anchors['top']:
		ancho=ancho-font.glyphs[item].layers[0].width+font.glyphs[item].layers[0].anchors['top'].x
	return ancho

def findIMatra(font,width,list):
	differences=[]
	for iMatra in list:
		iwidth=(font.glyphs[iMatra].layers[0].anchors['imatra'].x)-(font.glyphs[iMatra].layers[0].width)
...
def feature_pres(font,presList):
	...
	for c in listOfClassNames:
		if c.startswith('DevaFull.'):
			pos=len(c)-c.find('.')
			suffix=c[-pos:]
			code1+="sub [@iMatraBase]' [@"+c+"] by [iMatra-deva"+suffix+" iMatra_reph-deva"+suffix+" iMatra_reph_anusvara-deva"+suffix+"];\n"
	
	#segunda parte: combinaciones de anchos
	
	widths=[]
	for im in iMatrasList:
		widths.append((font.glyphs[im].layers[0].anchors['imatra'].x)-(font.glyphs[im].layers[0].width))
	devaHalfClasses=[]
	devaFullClasses=[]
	...
	for c1 in devaHalfClasses:
		for c2 in devaHalfClasses:
			for c3 in devaFullClasses:
				...
				suffix=replace[-pos:]
				code2+="sub [@iMatraBase]' [@"+c1+"] [@"+c2+"] [@"+c3+"] by ["+replace+" iMatra_reph-deva"+suffix+" iMatra_reph_anusvara-deva"+suffix+"];\n"
	...
	for c1 in devaHalfClasses:
		...
		for c2 in devaFullClasses:
			...
			replace=findIMatra(font,suma,iMatrasList)
			suffix=replace[-pos:]
			code2+="sub [@iMatraBase]' [@"+c1+"] [@"+c2+"] by ["+replace+" iMatra_reph-deva"+suffix+" iMatra_reph_anusvara-deva"+suffix+"];\n"
	...
	for item in presList:
		if checkExistence(font,item) and len(item)>2:
			...
			suffix=iMatra[-3:]
			glyphList=" ".join(item)
			code3+="sub [@iMatraBase]' "+glyphList+" by [iMatra-deva"+suffix+" iMatra_reph-deva"+suffix+" iMatra_reph_anusvara-deva"+suffix+"];\n"

	#cuarta parte: iiMatra
	code4=''
	if checkExistence(font,['iiMatra-deva','iiMatra_reph-deva','iiMatra_reph_anusvara-deva','iiMatra-deva.01']):
	...
	create_otfeature( featurename='pres', featurecode=code3+code2+code1+code4, codesig="Pre-Base Substitutions", targetfont=font)
...
def feature_abvs(font):
	...
	if checkExistence(font,base+['iMatra-deva','iMatra_reph-deva','iMatra_reph_anusvara-deva']):
		code+="""sub iMatra-deva reph-deva anusvara-deva by iMatra_reph_anusvara-deva;
		sub iMatra-deva reph-deva by iMatra_reph-deva;
"""
	if checkExistence(font,base+['iiMatra-deva','iiMatra_reph-deva','iiMatra_reph_anusvara-deva']):
...
"""
	if checkExistence(font,base+['oMatra-deva','oMatra_reph-deva','oMatra_anusvara-deva','oMatra_reph_anusvara-deva']):
...
"""
	if checkExistence(font,base+['auMatra-deva','auMatra_anusvara-deva','auMatra_reph-deva','auMatra_reph_anusvara-deva']):
...
"""
	if checkExistence(font,base+['eMatra-deva','eMatra_anusvara-deva','eMatra_reph-deva','eMatra_reph_anusvara-deva']):
...
"""
	if checkExistence(font,base+['aiMatra-deva','aiMatra_anusvara-deva','aiMatra_reph-deva','aiMatra_reph_anusvara-deva']):
...
"""
	if checkExistence(font,base+['rakar-deva','uMatra-deva','uuMatra-deva','rakar_uMatra-deva','rakar_uuMatra-deva']):
...
"""
	if checkExistence(font,base+['candraBindu-deva','candraBindu-deva.alt','anusvara-deva','anusvara-deva.alt']):
...
"""
	...
	create_otfeature( featurename='abvs', featurecode=code, codesig="Above-Base Substitutions", targetfont=font )
...
if classes==True:
	...
	createClass(font,'Deva',listFullforms(font)+listHalfforms(font)+listConjuncts(font))
	...
	createClass(font,'iMatra_reph_anusvara',listBegin(font,'iMatra_reph_anusvara-deva'))
...
if prefix==True:
	DevaRemoveReph(font,listFullforms(font)+listHalfforms(font)+listConjuncts(font))
