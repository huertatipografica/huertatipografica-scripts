...
for thisLayer in selectedLayers:
    ...
    compoundList = [
        ...
        if thisGlyphName
        in [c.componentName for c in g.layers[FontMaster.id].components]
        and (
            g.layers[FontMaster.id].width != thisLayer.width
            or len(g.layers[FontMaster.id].paths) > 0
        )
    ]
    if len(compoundList) > 1:
        ...
        Output += (
            "No compound glyphs with %s and different width" % thisGlyphName + "\n"
        )
