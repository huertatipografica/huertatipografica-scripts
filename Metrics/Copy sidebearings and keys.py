# MenuTitle: Copy SB and keys from...
...
class MetricsCopy(object):
    ...
    def __init__(self):
        ...
        self.w.text_anchor = vanilla.TextBox(
            (15, 12 + 2, 130, 14), "Copy metrics from:", sizeStyle="small"
        )
    ...
    def copyMetrics(self, sender):
        ...
        for thisLayer in Glyphs_selected:
            try:
                ...
                thisLayer.parent.leftMetricsKey = sourceGlyph.leftMetricsKey
                thisLayer.parent.rightMetricsKey = sourceGlyph.rightMetricsKey

                print("   ", thisLayer.LSB, "<-", glyphName, "->", thisLayer.RSB)
            except Exception as e:
                if (
                    "'objc.native_selector' object has no attribute 'name'" not in e
                ):  # CR in the selection string
                    print("Error:", e)
