...
class OpenTab(object):
    def __init__(self):
        ...
        self.w = vanilla.FloatingWindow(
            (360, 80),
            "Intercalation",
            autosaveName="com.juandelperal.Intercalation.mainwindow",
        )
        self.w.text = vanilla.TextBox(
            (13, 15, 320, 20),
            "Slashed string to intercal with selected glyphs (ex. /n/o)",
            sizeStyle="small",
        )
        self.w.interString = vanilla.EditText(
            (15, 35, 200, 20),
            "H",
            sizeStyle="small",
            placeholder="/n/n/n",
        ...
        self.w.go = vanilla.Button(
            (220, 30, 60, 30), "Go", sizeStyle="small", callback=self.Intercal
        )
    ...
    def SavePreferences(self, sender):
        Glyphs.defaults[
            "com.juandelperal.Intercalation.mainwindow.interString"
        ] = self.w.interString.get()

        return True

    def LoadPreferences(self):
        try:
            self.w.interString.set(
                Glyphs.defaults["com.juandelperal.Intercalation.mainwindow.interString"]
            )
        except:
            return False
