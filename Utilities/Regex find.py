...
class OpenTab(object):
    def __init__(self):
        self.w = vanilla.FloatingWindow(
            (360, 80), "Regex Find", autosaveName="com.juandelperal.regex.mainwindow"
        )
        self.w.text = vanilla.TextBox(
            (13, 15, 250, 20), "Regular expression string to search", sizeStyle="small"
        )
        ...
        self.w.go = vanilla.Button(
            (220, 30, 60, 30), "Find", sizeStyle="small", callback=self.regexFind
        )
    ...
    def SavePreferences(self, sender):
        Glyphs.defaults[
            "com.juandelperal.regex.mainwindow.searchFor"
        ] = self.w.searchFor.get()

        return True

    def LoadPreferences(self):
        try:
            self.w.searchFor.set(
                Glyphs.defaults["com.juandelperal.regex.mainwindow.searchFor"]
            )
        except:
            return False
    ...
    def regexFind(self, sender):
        ...
        if not searchFor:
            Message(
                "Hey!", "Type a regular expression for make the search.", OKButton="OK"
            )
        else:
